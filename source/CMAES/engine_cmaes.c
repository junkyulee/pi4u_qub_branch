...
void set_bounds( float **p_lower_bound, float **p_upper_bound, int dim);
void taskfun(float *x, int *pn, float *res, int *info);
int is_feasible(float *pop, float *lower_bound, float *upper_bound, int dim);

float load_pop_from_file( int step, float * const* pop, float *arFunvals, int dim, int lambda, int *checkp);

void make_all_points_feasible( cmaes_t *evo, float * const *pop, float * lower_bound, float * upper_bound );

float evaluate_population( cmaes_t *evo, float *arFunvals, float * const* pop, Density *d, int step );
void print_the_best( cmaes_t evo, int step );
void write_pop_to_file( cmaes_t evo, float *arFunvals, float * const* pop, int step );

int is_there_enough_time( float gt0, float dt );
...
void taskfun(float *x, int *n, float *res, int *info)
{
...
void set_bounds( float **p_lower_bound, float **p_upper_bound, int dim){
    ...
    float *lower_bound = malloc(dim*sizeof(float));
    float *upper_bound = malloc(dim*sizeof(float));
...
void print_the_best( cmaes_t evo, int step ){
		...
		const float *xbever = cmaes_GetPtr(&evo, "xbestever");
      	float        fbever = cmaes_Get(   &evo, "fbestever");
