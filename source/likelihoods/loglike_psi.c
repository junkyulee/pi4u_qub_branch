...
void loglike_psi_initialize( int nn ){
    ...
    logEv = (float *) malloc( theta_db.Nind*sizeof(float) );
    ...
    for( int p=0; p < theta_db.Nind; p++ ){
        ...
        for (int i = 0; i < theta_db.Nrec; i++)
            theta_data[p][i].x = (float*)malloc( Npr*sizeof(float));
...
//
//
float loglike_psi(float *psi, int n, void *output, int *info) {
    ...
    for( int p=0; p<theta_db.Nind; p++){
        ...
        for( int i = 0; i < theta_db.Nrec; i++ ){
            ...
            float log_pr_hb = prior_log_pdf( prior_tmp, Npr, theta_data[p][i].x );

            float log_pri = theta_data[p][i].logprior;
