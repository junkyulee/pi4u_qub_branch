...
static float theta[NTHETA][DIMTHETA];
static float psi[NPSI][DIMTHETA*2];
static float logev_theta;
static float theta_logprior[NTHETA];
static float denom[NPSI];
...
inline float priorHB(float *x, float *psi, int n) {
    float res = 1;

    /* for the truncated gaussian */
    float l[4] = {0.05, 3., 6, 1e-6};
    float u[4] = {10.0, 4., 15, 1};
...
void fitfun_init(int n) {
    ...
    static float B[NTHETA];  // exp(theta_logprior)
    static float A[NTHETA];
...
float posterior_theta(float *theta, int n, void *output, int *info) {
    float out = 0;
    float sum = 0;

    for (int i = 0; i < NPSI; i++){
        float pr_hb  = priorHB(theta, psi[i], n);
        sum += pr_hb/denom[i];
    }
    ...
    float loglike_theta = fitfun(theta, n, output, info);
