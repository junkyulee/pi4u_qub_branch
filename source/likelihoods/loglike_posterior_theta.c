...
void loglike_posterior_theta_initialize( ) {
    ...
    float **theta    = (float **)malloc( sizeof(float *) * db.Ntheta );
    theta[0] = (float * )malloc( sizeof(float) * db.Ntheta * db.Dtheta);
 	for( int i=0; i<db.Ntheta; i++)
        theta[i] = ( *theta + db.Dtheta * i);

    float **psi    = (float **)malloc( sizeof(float *) * db.Npsi );
    psi[0] = (float * )malloc( sizeof(float) * db.Npsi * db.Dpsi);
 	for( int i=0; i<db.Npsi; i++)
        psi[i] = ( *psi + db.Dpsi * i);

	float *theta_logprior = (float *) malloc( db.Ntheta * sizeof(float) );
	
	denom          = (float *) malloc( db.Npsi   * sizeof(float) );
    ...
    float A[db.Ntheta];
    float B[db.Ntheta];  // exp(theta_logprior)
...
//
//
float loglike_posterior_theta(float *theta, int n, void *output, int *info) {

	float out = 0;
	float sum = 0;

	for (int i = 0; i < db.Npsi; i++){
		float pr_hb  = exp( prior_log_pdf( priors[i], Npr, theta) );
		sum += pr_hb/denom[i];
	}
	...
	float loglike_theta = loglike_(theta, n, output, info);
