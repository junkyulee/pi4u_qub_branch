...
float mean_std( float *data, int N, float *mean, float *std );
...
void test_suite( Density *p, int N){
	...
	for(int i=0; i<N; i++){
		...
		float v1 = eval_density(p[i], 1.0);
		float v2 = eval_log_density(p[i], 1.0);
	...
	float x[4]={1,1,1,1};
	float v1 = prior_pdf( p, N, x);
	float v2 = prior_log_pdf( p, N, x);
	printf("	%lf - %lf \n", v1, exp(v2));

	printf("-----------\n");

	int Ns = 1e6;
	float rnd[Ns], mean, std, meanx, stdx;
