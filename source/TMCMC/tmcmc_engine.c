...
void initchaintask(float in_tparam[], int *pdim, float *out_tparam, int winfo[4])
{
	...
	float logprior = prior_log_pdf( priors, data.Nth, point );
        ...
        float *init_mean, float *chain_cov)
{
    ...
    for (step = 0; step < nsteps + burn_in; step++) {
        ...
        if (!fail){
            ...
            // Decide
            float logprior_candidate = prior_log_pdf( priors, data.Nth, candidate );
            float logprior_leader    = prior_log_pdf( priors, data.Nth, leader );
