...
void covupd(float *x, int start, int end, int npar, float w, float *xcov, float *xmean, float *wsum)
{
	...
	{ 
		...
		for (int i=0; i<n; i++)
		{
			...
			memcpy(xi, &x[i*p], p*sizeof(float));	//xi = x(i,:);
			*wsum   = w;

			memcpy(xmeann, xi, p*sizeof(float));	// xmeann = xi;
...
// $Revision: 1.0 $  $Date: $
	...
	float *lbounds = params.lbounds;
	float *ubounds = params.ubounds;
	...
	float drscale  = options.drscale; //getpar(options,'drscale',3);

	// scale for adapting the propsal
	float adascale = options.adascale; 	//getpar(options,'adascale',2.4/sqrt(npar));

	// blow factor for covariace update
	float qcovadj  = options.qcovadj;	//getpar(options,'qcovadj',1e-5);
	...
	float sigma2 = params.sigma2;	//getpar(params,'sigma2',1);
	...
	float *qcov = options.qcov;	//getpar(options,'qcov'); // proposal covariance
...
//	dodr = 0;
	...
	float *chain = (float *)calloc(1, nsimu*npar*sizeof(float));	//  = zeros(nsimu,npar);  // we store the chain here
	...
	float oldpar[npar];
	memcpy(oldpar, par0, npar*sizeof(float));

	//oldss        = feval(ssfun,oldpar,data);// first sum-of-squares
	//oldprior     = feval(priorfun,oldpar,params);
	float oldss    = ssfun(oldpar, npar);
	float oldprior = priorfun(oldpar, npar);
...
//		if ((isimu+1)/printint == (int)((isimu+1)/printint)) // info on every printint iteration
...
#endif
				memcpy(oldpar, newpar, npar*sizeof(float));  // oldpar = newpar;
				oldss    = newss;
				oldprior = newprior;
			}
			else
		...
		memcpy(&chain[isimu*npar], oldpar, npar*sizeof(float)); 	//chain(isimu,:) = oldpar; 
