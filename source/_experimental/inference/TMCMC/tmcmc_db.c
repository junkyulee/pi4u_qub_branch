...
void update_full_db(float point[], float F, float *G, int n, int surrogate)
{
	...
	if (full_db.entry[pos].point == NULL) full_db.entry[pos].point = (float *)malloc(data.Nth*sizeof(float));
...
void update_curgen_db(float point[], float F, float prior)
{
	...
	if (curgen_db.entry[pos].point == NULL) curgen_db.entry[pos].point = (float *)malloc(data.Nth*sizeof(float));
...
void update_curres_db(float point[EXPERIMENTAL_RESULTS], float F)
{
	int i, pos;
	...
	if (curres_db.entry[pos].point == NULL) curres_db.entry[pos].point = (float *)malloc((EXPERIMENTAL_RESULTS+1)*sizeof(float));
...
int load_curgen_db(int Gen)
{
	...
	for (pos = 0; pos < curgen_db.entries; pos++) {
		int i;
		for (i = 0; i < PROBDIM; i++) {
			if (curgen_db.entry[pos].point == NULL) curgen_db.entry[pos].point = (float *)malloc(PROBDIM*sizeof(float));
