...
//	int exist_pos_eig_values;
	...
	float current_eig_value, sum_neg_eig_values, min_pos_eig_value;
	float normalization_factor, small_pos_eig_value;

	float eps = 1e-12;	//set a value to place instead of zero or negative eigenvalues
	float zero = 1e-15;	//set a value to look for zero
...
//	a_psd = v * diag(max(diag(d), eps))/v;
	...
	int row;
//	float eps = 1e-3;  //set a value to place instead of zero or negative eigenvalues
	float eps = 1e-1;  //set a value to place instead of zero or negative eigenvalues
	...
	for(row=0; row<PROBDIM; row++){
		float eig_val = gsl_vector_get(eig_values, row);
...
//	min_limit = eps*10;
	...
	float fixrate = 0.01;
	int loops = 0;
	float min_limit = 1e-15;
	float convfix = 1 + fixrate;
...
//		printf("m = %f, neg = %f\n", m, neg);
		if (below_limit) {
			...
			for (i = 0; i < PROBDIM; i++) {
				float newv = gsl_matrix_get(nsigma, i, i);
				newv = newv + addit;
...
{
//	float current_hessian_copy[PROBDIM][PROBDIM]= {{0.0}};
	int row, column; 
...
void my_print_matrix_2d(char *msg, float **v, int PROBDIM)
{
	int n1 = PROBDIM;
	int n2 = PROBDIM;
	int i, j;
...
int make_posdef(float *mat, int dim, int method)
{
	int row, column; 
	int res = 1;
