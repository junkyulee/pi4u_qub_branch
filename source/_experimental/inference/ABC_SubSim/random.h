...
float gsl_ran_flat_logpdf(float x, float a, float b, int * isinf)
{
	float res = 0.0;
...
// code from http://lists.gnu.org/archive/html/help-gsl/2006-04/txtdb8Hdlx9uA.txt
float ran_mvn_pdf(const gsl_vector * x, const gsl_vector * mean, const gsl_matrix * covariance)
{
	int n = mean->size;
	...
	float det = gsl_linalg_LU_det(Sigma, s);
	...
	float res = exp(-0.5*sos)/sqrt(pow((2*M_PI),n)*det);
...
float ran_mvn_logpdf(const gsl_vector * x, const gsl_vector * mean, const gsl_matrix * covariance)
{
	int n = mean->size;
	...
	float res = -0.5*( sos + n*log(2*M_PI) + log(det) );
