...
int is_feasible(float *pop, int dim);
void taskfun(int *ptid, float *x, int *pn, int *info);


void callback(int *ptid, int *pnode_id, float *pout)
{
	int tid = *ptid;
...
float *lower_bound;	//float lower_bound[] = {-6.0, -6.0};
float *upper_bound;	//float upper_bound[] = {+6.0, +6.0};
...
void taskfun(int *ptid, float *x, int *pn, int *info)
{
	...
	float f = -fitfun(x, n, (void *)NULL, info);	/* CMA-ES needs this minus sign */
...
int main(int argn, char **args)
{
	...
	lower_bound = malloc(dim*sizeof(float));
	upper_bound = malloc(dim*sizeof(float));
	for (i = 0; i < dim; i++) {
		lower_bound[i] = -6;
		upper_bound[i] = +6;
	}
