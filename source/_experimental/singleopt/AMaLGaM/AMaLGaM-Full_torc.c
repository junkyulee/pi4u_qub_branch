...
float **matrixNew( int n, int m );
float vectorDotProduct( float *vector0, float *vector1, int n0 );
float *matrixVectorMultiplication( float **matrix, float *vector, int n0, int n1 );
float **matrixMatrixMultiplication( float **matrix0, float **matrix1, int n0, int n1, int n2 );
int blasDSWAP( int n, float *dx, int incx, float *dy, int incy );
int blasDAXPY(int n, float da, float *dx, int incx, float *dy, int incy);
void blasDSCAL( int n, float sa, float x[], int incx );
int linpackDCHDC( float a[], int lda, int p, float work[], int ipvt[] );
float **choleskyDecomposition( float **matrix, int n );
int linpackDTRDI( float t[], int ldt, int n );
float **matrixLowerTriangularInverse( float **matrix, int n );
void matrixWriteToFile( FILE *file, float **matrix, int n0, int n1 );
int *mergeSort( float *array, int array_size );
void mergeSortWithinBounds( float *array, int *sorted, int *tosort, int p, int q );
void mergeSortMerge( float *array, int *sorted, int *tosort, int p, int r, int q );
int *mergeSortFitness( float *objectives, float *constraints, int number_of_solutions );
void mergeSortFitnessWithinBounds( float *objectives, float *constraints, int *sorted, int *tosort, int p, int q );
void mergeSortFitnessMerge( float *objectives, float *constraints, int *sorted, int *tosort, int p, int r, int q );
...
float randomRealUniform01( void );
int randomInt( int maximum );
float random1DNormalUnit( void );
char *installedProblemName( int index );
int numberOfInstalledProblems( void );
float installedProblemLowerRangeBound( int index, int dimension );
float installedProblemUpperRangeBound( int index, int dimension );
short isParameterInRangeBounds( float parameter, int dimension );
void installedProblemEvaluation( int *pindex, float *parameters, float *objective_value, float *constraint_value, int *info );
void installedProblemEvaluationWithoutRotation( int index, float *parameters, float *objective_value, float *constraint_value, int *info );
void rosenbrockFunctionProblemEvaluation( float *parameters, float *objective_value, float *constraint_value, int *info );
float rosenbrockFunctionLowerRangeBound( int dimension );
float rosenbrockFunctionUpperRangeBound( int dimension );
...
float distanceInParameterSpace( float *solution_a, float *solution_b );
...
float *generateNewSolution( int population_index );
void adaptDistributionMultipliers( void );
short generationalImprovementForOnePopulation( int population_index, float *st_dev_ratio );
short betterFitness( float objective_value_x, float constraint_value_x, float objective_value_y, float constraint_value_y );
float getStDevRatio( int population_index, float *parameters );
...
float     tau,                              /* The selection truncation percentile (in [1/population_size,1]). */
...
float *matrixVectorMultiplication( float **matrix, float *vector, int n0, int n1 )
{
  ...
  result = (float *) malloc( n0*sizeof( float ) );
  for( i = 0; i < n0; i++ )
...
int blasDSWAP( int n, float *dx, int incx, float *dy, int incy )
{
  ...
  if (n > 0)
  {
    ...
    *dy   = dtmp;
    ...
    while( (--n) > 0 )
    {
      dx = (float *) ((char *) dx + incx);
      dy = (float *) ((char *) dy + incy);
...
int blasDAXPY(int n, float da, float *dx, int incx, float *dy, int incy)
{
  ...
  if( n > 0 && da != 0. )
  {
    ...
    *dy  += da * (*dx);
    ...
    while( n > 0 )
    {
      dy0   = (float *) ((char *) dy + incy);
      dy    = (float *) ((char *) dy0 + incy);
      dtmp0 = (*dy0);
      dtmp  = (*dy); 
      dx0   = (float *) ((char *) dx + incx); 
      dx    = (float *) ((char *) dx0 + incx);
      *dy0  = dtmp0 + da * (*dx0);
      *dy   = dtmp + da * (*dx);
      --n;
    }
...
/*-=-=-=-=-=-=-=-=-=-=-=-=-= Section Merge Sort -=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
/**
 * Sorts an array of floats and returns the sort-order (small to large).
 */
int *mergeSort( float *array, int array_size )
{
  int i, *sorted, *tosort;
...
void interpretCommandLine( int argc, char **argv )
{
  ...
  {
    tau                              = 0.35;
    population_size                  = (int) (17.0 + 3.0*pow((float) number_of_parameters,1.5));
...
void parseParameters( int argc, char **argv, int *index )
{
  ...
  noError = noError && sscanf( argv[*index+6], "%d", &population_size );
  noError = noError && sscanf( argv[*index+7], "%d", &number_of_populations );
...
void checkOptions( void )
{
  ...
  if( ((int) (tau*population_size)) <= 0 || tau >= 1 )
  {
    printf("\n");
    printf("Error: tau not in range (read: %e). Require tau in [1/pop,1] (read: [%e,%e]).", tau, 1.0/((float) population_size), 1.0);
    printf("\n\n");
...
/**
 * Returns a random float, distributed uniformly between 0 and 1.
 */
float randomRealUniform01( void )
{
  int64_t n26, n27;
  ...
  result               = (((int64_t)n26 << 27) + n27) / ((float) (1LLU << 53));
...
int randomInt( int maximum )
{
  ...
  result = (int) (((float) maximum)*randomRealUniform01());
  
  return( result );
}

/**
 * Returns a random float, distributed normally with mean 0 and variance 1.
 */
float random1DNormalUnit( void )
{
  float v1, v2, s, multiplier, value;
...
void initialize( void )
{
  ...
  alpha_AMS = 0.5*tau*(((float) population_size)/((float) (population_size-1)));
  delta_AMS = 2.0;
...
void initializeMemory( void )
{
  ...
  populations                      = (float ***) Malloc( number_of_populations*sizeof( float ** ) );
  ...
  objective_values                 = (float **) Malloc( number_of_populations*sizeof( float * ) );
  constraint_values                = (float **) Malloc( number_of_populations*sizeof( float * ) );
  ranks                            = (float **) Malloc( number_of_populations*sizeof( float * ) );
  selections                       = (float ***) Malloc( number_of_populations*sizeof( float ** ) );
  objective_values_selections      = (float **) Malloc( number_of_populations*sizeof( float * ) );
  constraint_values_selections     = (float **) Malloc( number_of_populations*sizeof( float * ) );
  mean_vectors                     = (float **) Malloc( number_of_populations*sizeof( float * ) );
  mean_vectors_previous            = (float **) Malloc( number_of_populations*sizeof( float * ) );
  covariance_matrices              = (float ***) Malloc( number_of_populations*sizeof( float ** ) );
  cholesky_factors_lower_triangle  = (float ***) Malloc( number_of_populations*sizeof( float ** ) );

  for( i = 0; i < number_of_populations; i++ )
  {
    populations[i] = (float **) Malloc( population_size*sizeof( float * ) );
    for( j = 0; j < population_size; j++ )
      populations[i][j] = (float *) Malloc( number_of_parameters*sizeof( float ) );

    populations_terminated[i] = 0;

    no_improvement_stretch[i] = 0;

    objective_values[i] = (float *) Malloc( population_size*sizeof( float ) );
    
    constraint_values[i] = (float *) Malloc( population_size*sizeof( float ) );

    ranks[i] = (float *) Malloc( population_size*sizeof( float ) );

    selections[i] = (float **) Malloc( selection_size*sizeof( float * ) );
    for( j = 0; j < selection_size; j++ )
      selections[i][j] = (float *) Malloc( number_of_parameters*sizeof( float ) );

    objective_values_selections[i] = (float *) Malloc( selection_size*sizeof( float ) );

    constraint_values_selections[i] = (float *) Malloc( selection_size*sizeof( float ) );

    mean_vectors[i] = (float *) Malloc( number_of_parameters*sizeof( float ) );

    mean_vectors_previous[i] = (float *) Malloc( number_of_parameters*sizeof( float ) );

    covariance_matrices[i] = (float **) Malloc( number_of_parameters*sizeof( float * ) );
    for( j = 0; j < number_of_parameters; j++ )
      covariance_matrices[i][j] = (float *) Malloc( number_of_parameters*sizeof( float ) );
    
    cholesky_factors_lower_triangle[i] = NULL;
  }

  lower_range_bounds = (float *) Malloc( number_of_parameters*sizeof( float ) );
  upper_range_bounds = (float *) Malloc( number_of_parameters*sizeof( float ) );
  lower_init_ranges  = (float *) Malloc( number_of_parameters*sizeof( float ) );
  upper_init_ranges  = (float *) Malloc( number_of_parameters*sizeof( float ) );
}
...
void initializeDistributionMultipliers( void )
{
  ...
  distribution_multipliers = (float *) Malloc( number_of_populations*sizeof( float ) );
  for( i = 0; i < number_of_populations; i++ )
...
void initializePopulationsAndFitnessValues( void )
{
  ...
  float *distances, d, d_min, **solutions, *fitnesses, *constraints, **leader_vectors;
...
//      installedProblemEvaluation( &problem_index, populations[i][j], &(objective_values[i][j]), &(constraint_values[i][j]), NULL );
        int info[5];
        info[0] = 0;		             // start
        info[1] = 0;                            // initialize
  ...
  if( number_of_populations > 1 )
  {
    ...
    solutions                  = (float **) Malloc( ssize*sizeof( float * ) );
    fitnesses                  = (float *) Malloc( ssize*sizeof( float ) );
    constraints                = (float *) Malloc( ssize*sizeof( float ) );
    temporary_population_sizes = (int *) Malloc( ssize*sizeof( int ) );
    distances                  = (float *) Malloc( ssize*sizeof( float ) );
    leader_vectors             = (float **) Malloc( number_of_populations*sizeof( float * ) );

    for( i = 0; i < ssize; i++ )
      solutions[i] = (float *) Malloc( number_of_parameters*sizeof( float ) );

    for( i = 0; i < number_of_populations; i++ )
      leader_vectors[i] = (float *) Malloc( number_of_parameters*sizeof( float ) );
...
void initializeObjectiveRotationMatrix( void )
{
  ...
  float **matrix, **product, theta, cos_theta, sin_theta;

  if( rotation_angle == 0.0 )
    return;

  matrix = (float **) Malloc( number_of_parameters*sizeof( float * ) );
  for( i = 0; i < number_of_parameters; i++ )
    matrix[i] = (float *) Malloc( number_of_parameters*sizeof( float ) );

  rotation_matrix = (float **) Malloc( number_of_parameters*sizeof( float * ) );
  for( i = 0; i < number_of_parameters; i++ )
    rotation_matrix[i] = (float *) Malloc( number_of_parameters*sizeof( float ) );
...
void writeGenerationalStatistics( void )
{
  int     i, j;
  char    string[1000];
  float  overall_objective_avg, overall_objective_var, overall_objective_best, overall_objective_worst,
  ...
  population_objective_avg    = (float *) Malloc( number_of_populations*sizeof( float ) );
  population_constraint_avg   = (float *) Malloc( number_of_populations*sizeof( float ) );
  population_objective_var    = (float *) Malloc( number_of_populations*sizeof( float ) );
  population_constraint_var   = (float *) Malloc( number_of_populations*sizeof( float ) );
  population_objective_best   = (float *) Malloc( number_of_populations*sizeof( float ) );
  population_constraint_best  = (float *) Malloc( number_of_populations*sizeof( float ) );
  population_objective_worst  = (float *) Malloc( number_of_populations*sizeof( float ) );
  population_constraint_worst = (float *) Malloc( number_of_populations*sizeof( float ) );
  ...
  overall_objective_avg = overall_objective_avg / ((float) (number_of_populations*population_size));
  overall_constraint_avg = overall_constraint_avg / ((float) (number_of_populations*population_size));
  ...
  overall_objective_var = overall_objective_var / ((float) (number_of_populations*population_size));
  overall_constraint_var = overall_constraint_var / ((float) (number_of_populations*population_size));
  ...
  for( i = 0; i < number_of_populations; i++ )
  {
    ...
    population_objective_avg[i]  = population_objective_avg[i] / ((float) population_size);
    population_constraint_avg[i] = population_constraint_avg[i] / ((float) population_size);
    ...
    population_objective_var[i]  = population_objective_var[i] / ((float) population_size);
    population_constraint_var[i] = population_constraint_var[i] / ((float) population_size);
...
void makeSelectionsForOnePopulationUsingDiversityOnRank0( int population_index )
{
  ...
  float *nn_distances, distance_of_farthest, value;
  ...
  nn_distances = (float *) Malloc( number_of_rank0_solutions*sizeof( float ) );
...
void estimateMeanVectorML( int population_index )
{
  ...
  for( i = 0; i < number_of_parameters; i++ )
  {
    ...
    mean_vectors[population_index][i] /= (float) selection_size;
  }
...
void estimateCovarianceMatrixML( int population_index )
{
  ...
  for( i = 0; i < number_of_parameters; i++ )
  {
    for( j = i; j < number_of_parameters; j++ )
    {
      ...
      covariance_matrices[population_index][i][j] /= (float) selection_size;
    }
  }
...
void generateAndEvaluateNewSolutionsToFillPopulations( void )
{
  ...
  float *solution, *solution_AMS, shrink_factor;

  solution_AMS = (float *) Malloc( number_of_parameters*sizeof( float ) );
...
float *generateNewSolution( int population_index )
{
  ...
  do
  {
    ...
    {
      result = (float *) Malloc( number_of_parameters*sizeof( float ) );
      for( i = 0; i < number_of_parameters; i++ )
        result[i] = lower_init_ranges[i] + (upper_init_ranges[i] - lower_init_ranges[i])*randomRealUniform01();
    }
    else
    {
      z = (float *) Malloc( number_of_parameters*sizeof( float ) );
...
void adaptDistributionMultipliers( void )
{
  ...
  for( i = 0; i < number_of_populations; i++ )
  {
    if( !populations_terminated[i] )
    {
      if( (((float) out_of_bounds_draws[i])/((float) samples_drawn_from_normal[i])) > 0.9 )
...
short generationalImprovementForOnePopulation( int population_index, float *st_dev_ratio )
{
  ...
  float *average_parameters_of_improvements;
  ...
  average_parameters_of_improvements = (float *) Malloc( number_of_parameters*sizeof( float ) );
  for( i = 0; i < number_of_parameters; i++ )
  ...
  *st_dev_ratio = 0.0;
  ...
  {
    for( i = 0; i < number_of_parameters; i++ )
      average_parameters_of_improvements[i] /= (float) number_of_improvements;
