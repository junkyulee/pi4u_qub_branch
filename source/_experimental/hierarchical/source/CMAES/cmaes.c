...
/* --- Changes : --- 
   03/03/21: argument const float *rgFunVal of
...
float cmaes_random_Gauss(cmaes_random_t *); /* (0,1)-normally distributed */
float cmaes_random_Uniform(cmaes_random_t *);
...
float cmaes_timings_update(cmaes_timings_t *timing);
void   cmaes_timings_tic(cmaes_timings_t *timing);
float cmaes_timings_toc(cmaes_timings_t *timing);

void cmaes_readpara_init (cmaes_readpara_t *, int dim, const float * xstart, 
    const float * sigma, int seed, int lambda, const char * filename);
...
const float * cmaes_Optimize( cmaes_t *, float(*pFun)(float const *, int dim), 
        long iterations);
float const * cmaes_SetMean(cmaes_t *, const float *xmean);
float * cmaes_PerturbSolutionInto(cmaes_t *t, float *xout, 
        float const *xin, float eps);
...
static void Eigen( int N,  float **C, float *diag, float **Q, 
        float *rgtmp);
static int  Check_Eigen( int N,  float **C, float *diag, float **Q);
static void QLalgo2 (int n, float *d, float *e, float **V); 
static void Householder2(int n, float **V, float *d, float *e); 
...
static void   Sorted_index( const float *rgFunVal, int *index, int n);
static int    SignOfDiff( const void *d1, const void * d2);
static float douSquare(float);
static float rgdouMax( const float *rgd, int len);
static float rgdouMin( const float *rgd, int len);
static float douMax( float d1, float d2);
static float douMin( float d1, float d2);
static int    intMin( int i, int j);
static int    MaxIdx( const float *rgd, int len);
static int    MinIdx( const float *rgd, int len);
static float myhypot(float a, float b);
static float * new_float( int n);
...
float * cmaes_init_final(cmaes_t *t /* "this" */)
{
    int i, j, N;
    float dtest, trace;
    ...
    t->chiN = sqrt((float) N) * (1. - 1./(4.*N) + 1./(21.*N*N));
    ...
    t->rgpc = new_float(N);
    t->rgps = new_float(N);
    t->rgdTmp = new_float(N+1);
    t->rgBDz = new_float(N);
    t->rgxmean = new_float(N+2); t->rgxmean[0] = N; ++t->rgxmean;
    t->rgxold = new_float(N+2); t->rgxold[0] = N; ++t->rgxold; 
    t->rgxbestever = new_float(N+3); t->rgxbestever[0] = N; ++t->rgxbestever; 
    t->rgout = new_float(N+2); t->rgout[0] = N; ++t->rgout;
    t->rgD = new_float(N);
    t->C = (float**)new_void(N, sizeof(float*));
    t->B = (float**)new_void(N, sizeof(float*));
    t->publicFitness = new_float(t->sp.lambda); 
    t->rgFuncValue = new_float(t->sp.lambda+1); 
    t->rgFuncValue[0]=t->sp.lambda; ++t->rgFuncValue;
    t->arFuncValueHist = new_float(10+(int)ceil(3.*10.*N/t->sp.lambda)+1);
    t->arFuncValueHist[0] = (float)(10+(int)ceil(3.*10.*N/t->sp.lambda));
    t->arFuncValueHist++; 
    ...
    t->rgrgx = (float **)new_void(t->sp.lambda, sizeof(float*));
    for (i = 0; i < t->sp.lambda; ++i) {
        t->rgrgx[i] = new_float(N+2);
        t->rgrgx[i][0] = N; 
        t->rgrgx[i]++;
    }
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
float * cmaes_init(cmaes_t *t, /* "this" */
    int dimension, 
    float *inxstart,
    float *inrgstddev, /* initial stds */
    long int inseed,
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
float const * cmaes_SetMean(cmaes_t *t, const float *xmean)
    /*
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
float const * cmaes_ReSampleSingle_old(cmaes_t *t, float *rgx)
{
    int i, j, N=t->sp.N;
    float sum; 

    if (rgx == NULL)
        FATAL("cmaes_ReSampleSingle(): Missing input float *x",0,0,0);
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
float * const * cmaes_ReSampleSingle(cmaes_t *t, int iindex)
{
    int i, j, N=t->sp.N;
    float *rgx; 
    float sum; 
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
float * cmaes_PerturbSolutionInto(cmaes_t *t, float *rgx, float const *xmean, float eps)
{
    int i, j, N=t->sp.N;
    float sum; 
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
const float * cmaes_Optimize(cmaes_t *evo, float(*pFun)(float const *, int dim), long iterations)
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
float * cmaes_UpdateDistribution(cmaes_t *t, const float *rgFunVal)
{
    ...
    if(psxps/N > 1.5 + 10.*sqrt(2./N) 
        ...
        float tfac = sqrt((1 + douMax(0, log(psxps/N))) * N / psxps);
        for (i=0; i<N; ++i) 
            t->rgps[i] *= tfac;
        psxps *= tfac*tfac; 
    }
...
static void Adapt_C2(cmaes_t *t, int hsig)
{
    ...
    if (t->sp.ccov != 0. && t->flgIniphase == 0) {
        ...
        float ccov1 = douMin(t->sp.ccov * (1./t->sp.mucov) * (flgdiag ? (N+1.5) / 3. : 1.), 1.);
        float ccovmu = douMin(t->sp.ccov * (1-1./t->sp.mucov)* (flgdiag ? (N+1.5) / 3. : 1.), 1.-ccov1); 
        float sigmasquare = t->sigma * t->sigma; 
...
{ 
    ...
    while (key < keyend)
    {
        ...
        if (strncmp(key, "diag(D)", 7) == 0)
        {
            for (i = 0; i < N; ++i)
                t->rgdTmp[i] = t->rgD[i];
            qsort(t->rgdTmp, (unsigned) N, sizeof(float), &SignOfDiff); /* superfluous */
            for (i = 0; i < N; ++i)
        ...
        { 
            ...
            for (i = 0; i < N; ++i)
                t->rgdTmp[i] = t->rgD[i];
            qsort(t->rgdTmp, (unsigned) N, sizeof(float), &SignOfDiff);
...
/* --------------------------------------------------------- */
float cmaes_Get( cmaes_t *t, char const *s)
{
    int N=t->sp.N;
...
/* --------------------------------------------------------- */
float * cmaes_GetInto( cmaes_t *t, char const *s, float *res)
{
    int i, N = t->sp.N;
    float const * res0 = cmaes_GetPtr(t, s);
    if (res == NULL)
        res = new_float(N);
    for (i = 0; i < N; ++i)
        res[i] = res0[i];
...
/* --------------------------------------------------------- */
float * cmaes_GetNew( cmaes_t *t, char const *s)
{
    return (cmaes_GetInto(t, s, NULL));
}

/* --------------------------------------------------------- */
const float * cmaes_GetPtr( cmaes_t *t, char const *s)
{
    int i, N=t->sp.N;
    ...
    */
{
    ...
    int flgprinted = 0;
    int flgwritten = 0; 
    float deltaprinttime = (float)(time(NULL)-t->printtime); /* using clock instead might not be a good */
    float deltawritetime = (float)(time(NULL)-t->writetime); /* idea as disc time is not CPU time? */
    float deltaprinttimefirst = (float)(t->firstprinttime ? time(NULL)-t->firstprinttime : 0); /* time is in seconds!? */
    float deltawritetimefirst = (float)(t->firstwritetime ? time(NULL)-t->firstwritetime : 0); 
    ...
    {
        deltaprinttime = (float)time(NULL); /* forces printing */
        deltawritetime = (float)time(NULL);
    }
...
/* ========================================================= */
static void Eigen( int N,  float **C, float *diag, float **Q, float *rgtmp)
    /* 
       ...
       */
{
    ...
    if (rgtmp == NULL) /* was OK in former versions */
        FATAL("cmaes_t:Eigen(): input parameter float *rgtmp must be non-NULL", 0,0,0);
...
float cmaes_timings_update(cmaes_timings_t *t)
{
    ...
    diffc = (float)(t->lastclock - lc) / CLOCKS_PER_SEC; /* is presumably in [-21??, 21??] */
...
float cmaes_timings_toc(cmaes_timings_t *t)
{
    if (!t->istic) {
...
/* --------------------------------------------------------- */
float cmaes_random_Gauss(cmaes_random_t *t)
{
...
/* --------------------------------------------------------- */
float cmaes_random_Uniform( cmaes_random_t *t)
{
    long tmp;
    ...
    return (float)(t->aktrand)/(2.147483647e9);
}
...
void cmaes_readpara_init (cmaes_readpara_t *t,
        int dim, 
        const float * inxstart, 
        const float * inrgsigma,
        int inseed, 
        int lambda, 
        const char * filename)
{
    ...
    t->rgp2adr = (float ***) new_void(11, sizeof(float **));
    ...
    for (ipara=1; ipara < t->n1outpara; ++ipara) {
        ...
        if (t->rgsformat[ipara][len-1] == 'd') /* read integer */
            ...
            if (strncmp(" fac*", t->rgsformat[ipara], 5) == 0) {
                fprintf(fp, " ");
                fprintf(fp, t->rgsformat[ipara]+5, *(float *)t->rgpadr[ipara]);
            } else
                fprintf(fp, t->rgsformat[ipara], *(float *)t->rgpadr[ipara]);
        }
        fprintf(fp, "\n");
    } /* for */
    fprintf(fp, "\n");
     ...
     */
{
    ...
    if (t->diagonalCov == -1)
        t->diagonalCov = 2 + 100. * N / sqrt((float)t->lambda); 
    ...
    if (t->stopMaxIter == -1)
        t->stopMaxIter = ceil((float)(t->stopMaxFunEvals / t->lambda)); 
    ...
    t->damps = t->damps 
        ...
        * douMax(0.3, 1. -                                       /* modify for short runs */
                (float)N / (1e-6+douMin(t->stopMaxIter, t->stopMaxFunEvals/t->lambda))) 
        + t->cs;                                                 /* minor increment */

    if (t->updateCmode.modulo < 0)
        t->updateCmode.modulo = 1./t->ccov/(float)(N)/10.;
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
static float douSquare(float d)
{
    return d*d;
}
...
static float douMax(float i, float j)
{
    return i > j ? i : j;
}

static float douMin(float i, float j)
{
    return i < j ? i : j;
}
...
#if 1
/* dirty index sort */
static void Sorted_index(const float *rgFunVal, int *iindex, int n)
{
    int i, j;
...
{
    static char s[170];
    float *p = (float *) calloc((unsigned) n, sizeof(float));
    if (p == NULL) {
        sprintf(s, "new_float(): calloc(%ld,%ld) failed",
                (long)n,(long)sizeof(float));
        FATAL(s,0,0,0);
    }
    return p;
}
