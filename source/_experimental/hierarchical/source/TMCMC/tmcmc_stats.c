...
float Objlogp(float x, float *fj, int fn, float pj, float tol)
{
	...
	float *weight = (float *)malloc(fn*sizeof(float));
#else
	float weight[fn];
#endif
	for (i = 0; i < fn; i++)
...
#ifdef LARGE_SCALE_POPS
	float *q = (float *)malloc(fn*sizeof(float));
#else
	float q[fn];
#endif
...
/*** OPTIMIZATION ***/
int fzerofind(float *fj, int fn, float pj, float tol, float *xmin, float *fmin)
{
	size_t iter = 0;
	/*size_t max_iter = data.options.MaxIter;*/	/* USER input - not used here */
	float Tol = data.options.Tol;
	int Display = data.options.Display;
	float Step = data.options.Step;
	float x_lo = 0.0, x_hi = 4.0;
	int conv = 0;
...
retry:
	...
	{
		float x = x_lo + iter*Step;
		float fx = Objlogp(x, fj, fn, pj, tol);
...
int fzerofind1(float *fj, int fn, float pj, float tol, float *xmin, float *fmin)
{
	size_t iter = 0;
	/*size_t max_iter = data.options.MaxIter;*/	/* USER input - not used here */
	float Tol = data.options.Tol;
...
int fminsearch(float *fj, int fn, float pj, float tol, float *xmin, float *fmin)
{
...
int fmincon(float *fj, int fn, float pj, float tol, float *xmin, float *fmin)
{
	int status;
...
/*	T = gsl_min_fminimizer_quad_golden;*/
	...
	float f_lo = Objlogp_gsl(x_lo, &fp);
	float f_hi = Objlogp_gsl(x_hi, &fp);
	if (f_lo < f_hi) {
		m = x_lo;
		fm = f_lo;
	} else {
	...
	for (i = 0; i < max_iter; i++) {
		float x = x_lo + i*(x_hi-x_lo)/max_iter;
		float fx = Objlogp_gsl(x, &fp);
		if (fx < fm) {
			m = x;
			fm = fx;
		}
...
void calculate_statistics(float flc[], int n, int nselections, int gen, unsigned int sel[])
{
	int Display = data.options.Display;
	/*float pflag = 0;*/
	float tolCOV = data.TolCOV;
	float *CoefVar = runinfo.CoefVar;
	float *p = runinfo.p;
	int *Num = data.Num;
/*	int *currentuniques = runinfo.currentuniques;*/
	float *logselection = runinfo.logselection;
	float Step = data.options.Step;
	float fmin = 0, xmin = 0;
	int conv = 0;
#if 1
...
*/
	...
	int i;
#if 1
	float *flcp = (float *)malloc(n*sizeof(float));
	for (i= 0; i<n; i++)
		flcp[i] = flc[i]*(p[j]-p[j-1]);


	float fjmax= compute_max (flcp,n );
