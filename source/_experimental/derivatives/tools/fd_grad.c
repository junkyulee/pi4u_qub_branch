...
int romberg(float StepRatio, float der_init[], int N, float rombexpon[], int R, float *der_romb, float *errest);
int derivest(int id, float *der_init, int Nder, float H0, float StepRatio);
void pndlga_task(int *pid, float *X, int *pN, float *XL, float *XU, float *ph, int *pIORD, float *GRAD);
...
void run(float *TP, int *pn, float *res)
{
	*res = F(TP, pn);
	return;
}
...
void do_mytest()
{
	...
	float *XL = data.lowerbound;
	float *XU = data.upperbound;
	float *X = data.theta;
	int IORD = data.iord;
	int N = data.Nth;

	float GRADs[MAXGRADS][data.Nth];		// grouped by point  
	float allGRADs[data.Nth][MAXGRADS];	// grouped by direction
	float H[MAXGRADS];
...
//	float step = 1.5;
	...
	for (k = 0; k < MAXGRADS; k++) {
		float h = data.h0*pow(data.stepratio, -k);
...
int derivest(int id, float *der_init, int Nder, float H0, float StepRatio)
{
	...
	float *rombexpon;
	if (R > 0)
		rombexpon = malloc(R*sizeof(float));
	else
		rombexpon = NULL;
	...
	float best_errest = 1e10;
	float best_stepsize = 0;
	float best_derest = 1e10;
	
	for (p = 0; p <= Nder-N; p++) 
	{
		float stepsize = H0*pow(StepRatio, -p);
