...
int main (int argc, char **argv)
{
    ...
    float gt0, gt1, gt2;	// used for clean restart mechanism
    int i;
    FILE *fpt1;
    FILE *fpt2;
    FILE *fpt3;
    ...
    if (nbin != 0)
    {
        ...
        min_binvar = (float *)malloc(nbin*sizeof(float));
        max_binvar = (float *)malloc(nbin*sizeof(float));
        for (i=0; i<nbin; i++)
        {
