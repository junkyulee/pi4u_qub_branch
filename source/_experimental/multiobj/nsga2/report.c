...
void dump_pop (population *pop, FILE *fpt)
{
    for (int i=0; i<popsize; i++)
    {
        for (int j=0; j<nobj; j++)
        {
            fwrite(&pop->ind[i].obj[j], sizeof(float), 1, fpt);
        }
        if (nreal!=0)
        {
            for (int j=0; j<nreal; j++)
            {
		fwrite(&pop->ind[i].xreal[j], sizeof(float), 1, fpt);
            }
        }
	fwrite(&pop->ind[i].constr_violation, sizeof(float), 1, fpt);
    }
    return;
}
...
void load_pop (population *pop, FILE *fpt)
{
    for (int i=0; i<popsize; i++)
    {
        for (int j=0; j<nobj; j++)
        {
            fread(&pop->ind[i].obj[j], sizeof(float), 1, fpt);
        }
        if (nreal!=0)
        {
            for (int j=0; j<nreal; j++)
            {
		fread(&pop->ind[i].xreal[j], sizeof(float), 1, fpt);
            }
        }
	fread(&pop->ind[i].constr_violation, sizeof(float), 1, fpt);
    }
    return;
}
