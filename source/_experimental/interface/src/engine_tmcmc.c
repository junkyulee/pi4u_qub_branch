...
void data_init(tmcmc_data_t *tmcmc_data, int Nth, int MaxStages, int PopSize, float *lb, float *ub)
{
	int i;
	...
	tmcmc_data->data.lowerbound = malloc(tmcmc_data->data.Nth*sizeof(float));
	tmcmc_data->data.upperbound = malloc(tmcmc_data->data.Nth*sizeof(float));
	...
	tmcmc_data->data.Num = malloc(tmcmc_data->data.MaxStages*sizeof(float));
	...
	tmcmc_data->runinfo.CoefVar = calloc(1, tmcmc_data->data.MaxStages*sizeof(float));
	tmcmc_data->runinfo.p = calloc(1, tmcmc_data->data.MaxStages*sizeof(float));
	tmcmc_data->runinfo.currentuniques = calloc(1, tmcmc_data->data.MaxStages*sizeof(float));
	tmcmc_data->runinfo.logselection = calloc(1, tmcmc_data->data.MaxStages*sizeof(float));
	tmcmc_data->runinfo.acceptance = calloc(1, tmcmc_data->data.MaxStages*sizeof(float));

	float *SSmem = (float *)calloc(1, tmcmc_data->data.Nth*tmcmc_data->data.Nth*sizeof(float));
	tmcmc_data->runinfo.SS = (float **)malloc(tmcmc_data->data.Nth*sizeof(float *));
	for (i = 0; i < tmcmc_data->data.Nth; i++) {
		tmcmc_data->runinfo.SS[i] = SSmem + i*tmcmc_data->data.Nth; //&SSmem[i*data.Nth];
	}

	tmcmc_data->runinfo.meantheta = calloc(1, tmcmc_data->data.MaxStages*sizeof(float *));
	for (i = 0; i < tmcmc_data->data.MaxStages; i++) {
		tmcmc_data->runinfo.meantheta[i] = calloc(1, tmcmc_data->data.Nth*sizeof(float));
	}
...
void torc_update_curgen_db_task(float point[], float *pF, tmcmc_data_t *tmcmc_data, int *tmcmc_owner)
{
	float F = *pF;
...
void torc_update_curgen_db(float point[], float F, int dim, tmcmc_data_t *tmcmc_data, int *tmcmc_owner)
{
#ifdef USE_TORC
	int me = torc_node_id();
#else
...
#if 0
typedef float (*feval_t)(float *x, long n, void *output, int *info);
feval_t fitfun;
#else
float fitfun(float *TP, long N, void *output, int *info);
#endif
...
void evaluate_F(float point[], int dim, float *Fval, int gen_id, int chain_id, int step_id, int ntasks)
{
	int i;
	float G[64], F;	// maxtasks
	int winfo[4];
...
void initchaintask(float in_tparam[], int *pdim, float *out_tparam, int winfo[4], tmcmc_data_t *tmcmc_data, int *tmcmc_owner)
{
	int i;
	int dim = *pdim;
...
void compute_candidate(float candidate[], float leader[], int dim, float var, float *bSS, float *lowerbound, float *upperbound)
{
...
retry:
	mvnrnd(leader, (float *)bSS, candidate, dim);
	for (i = 0; i < dim; i++) {
...
void chaintask(float in_tparam[], int *pdim, int *pnsteps, float *out_tparam, int winfo[4], float *ppj, float *bSS, float *lowerbound, float *upperbound,
...
//	int tmcmc_owner = *ptmcmc_owner;
	...
	float leader[dim], fleader, fpc_leader;		// fold
	float candidate[dim], fcandidate, fpc_candidate;	// fnew
...
//	float pj = tmcmc_data.runinfo.p[tmcmc_data.runinfo.Gen];
	float pj = *ppj;
	...
	for (step = 0; step < nsteps; step++) {
		...
		/* Decide */
		float prior_candidate = priorpdf(candidate, dim, lowerbound, upperbound);	// from PanosA 
		float prior_leader = priorpdf(leader, dim, lowerbound, upperbound);
		float L = exp((prior_candidate-prior_leader)+(fpc_candidate-fpc_leader)*pj);
		if (L > 1) L = 1;
...
//	data_init(tmcmc_data);
	...
	float out_tparam[tmcmc_data->data.PopSize];	// nchains
	for (i = 0; i < nchains; i++) {
		...
		int d;
		float in_tparam[tmcmc_data->data.Nth];
...
//	print_curgen_db();
	...
	for (i = 0; i < tmcmc_data->data.PopSize; i++) {
		leaders[i].point = calloc(1, tmcmc_data->data.Nth*sizeof(float));
	}
...
extern float fitfun(float *TP, long n, void *output, int *info);
#endif
