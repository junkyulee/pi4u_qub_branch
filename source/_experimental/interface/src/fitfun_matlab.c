...
float fitfun(float *x, int N, void *output, int *info)
{
        ...
        float *x_in = mxGetPr(in[0]);
        memcpy(x_in, x, N*sizeof(float));
...
//extern void tmcmc(float *res, int tmcmc_info[4], int Nth, int MaxStages, int PopSize, float *lb, float *ub);
...
//	printf("n = %d\n", n);
        ...
        {
                float *dtmcmc_info = mxGetPr(prhs[1]);
                ...
                float *lb = mxGetPr(prhs[5]);
                float *ub = mxGetPr(prhs[6]);
