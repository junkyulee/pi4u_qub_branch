...
float fitfun(float *x, long N, void *output, int *info)
{
...
//	source("fitfun.R");
	source(fitfun_file);
	float r = R_fitfun(x, N);
	return r;
}

#include "engine_tmcmc.h"

void tmcmcR(float *res, int *tmcmc_info, int *pNth, int *pMaxStages, int *pPopSize, float *lb, float *ub)
{
	int Nth = *pNth;
